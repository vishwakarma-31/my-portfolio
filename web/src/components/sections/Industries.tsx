...
export function Industries() {
  ...
  return (
    <section ref={container} className="dark relative z-10 min-h-[100vh] py-32 bg-[#0d0d0d] text-white overflow-hidden border-t border-[#333] flex flex-col justify-center">
      
      <div className="px-6 md:px-12 xl:px-24 mb-12 max-w-[1280px] mx-auto w-full">
        <h2 className="type-h2 font-extralight text-white tracking-[-0.01em] mb-6">
          Domain Agnostic
        </h2>
        <p className="type-body leading-relaxed max-w-[760px]">
        ...
        <div ref={scrollTrack} className="flex gap-8 w-max pb-12 pr-6 md:pr-12 xl:pr-24">
          {industries.map((ind, i) => (
            <Card 
              key={i} 
              variant="interactive"
              className="ind-card opacity-0 w-[300px] bg-[rgba(255,255,255,0.02)] border-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.04)] flex flex-col shrink-0"
            >
              <CardContent className="p-8 flex flex-col h-full">
                ...
                <div className="w-full h-[120px] mb-8 bg-background-elevated rounded-sm relative overflow-hidden flex items-center justify-center shrink-0">
                   <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white to-transparent blur-[10px]" />
                   <span className="type-tiny text-white/30 uppercase tracking-[0.08em] z-10">{ind.name}</span>
                </div>

                <h3 className="type-body-large text-white font-medium mb-6">
                  {ind.name}
                </h3>
                ...
                <div className="mt-auto">
                  ...
                  <div className="type-caption text-white leading-[1.5]">{ind.sol}</div>
                </div>
              </CardContent>
            </Card>
          ))}
